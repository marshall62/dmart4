...
function checkAndAddIntField(
  ...
  if (formData.get(field)) {
    const f = field as keyof SelectArtwork;
    // @ts-expect-error formData get returns FormDataEntryValue which is a union type
    rec[f] = intOrNull(formData.get(field));
  }
}
...
function formDataToPatchRecord(formData: FormData): Partial<SelectArtwork> {
  const rec = {};
  ...
  return rec as Partial<SelectArtwork>;
}
...
export async function PATCH(request: Request) {
  const checkResult = await checkCookieHeader();
  if (!checkResult) {
  ...
  const thumbnail_file = form.has("thumbnailFile")
    ? (form.get("thumbnailFile") as File)
    : null;
  const midsize_file = form.has("midsizeFile")
  ...
  if (Object.keys(new_data).length !== 0) await updateArtwork(artId, new_data);
  ...
  // tags are sent
  if (tags.length > 0) await updateArtworkTags(artId, tags);
  const artwork = await getArtwork(artId);
...
export async function POST(request: Request) {
  ...
  const tags = formDataTags(form);
  if (tags.length > 0) await updateArtworkTags(artworkId, tags);
...
export async function DELETE(request: Request) {
  ...
  if (artwork) {
    ...
    if (artwork.thumbnail_image_url)
      await deleteBlob(artwork.thumbnail_image_url);
